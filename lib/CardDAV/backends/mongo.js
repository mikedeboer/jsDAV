...
var jsCardDAV_Backend_Mongo = module.exports = jsCardDAV_iBackend.extend({
    ...
    createAddressBook: function(principalUri, url, properties, callback) {
        ...
        self.mongo.collection(self.addressBooksTableName).find({}).sort({uid: -1}).limit(1).toArray(function(err, docs) {
    ...
    getCards: function(addressbookId, callback) {
        ...
        self.mongo.collection(self.cardsTableName).find({addressbookid: addressbookId}).toArray(function(err, docs) {
	        if(err)
	        	return callback(err)
            else if (!docs || !docs.length) {
                return callback(null, []);
    ...
    getCard: function(addressBookId, cardUri, callback) {
        ...
        self.mongo.collection(self.cardsTableName).findOne({uri: cardUri},
        	function(err, doc) {
		        if(err)
		        	return callback(err);
		        else if(!doc)
    ...
    updateCard: function(addressBookId, cardUri, cardData, callback) {
        ...
        self.mongo.collection(self.cardsTableName).update(
        	{uri: cardUri},
        	{$set: {
	        	carddata: cardData,
	        	lastmodified: now
    ...
    deleteCard: function(addressBookId, cardUri, callback) {
        ...
        self.mongo.collection(self.cardsTableName).remove({uri: cardUri}, function(err) {
	        if(err) return callback(err);
	        else {
	       		self.mongo.collection(self.addressBooksTableName).update(
	       			{uid: addressBookId},
