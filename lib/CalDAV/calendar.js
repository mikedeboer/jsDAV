...
(function() {
    this.implement(jsDAV_CalDAV_iCalendar, jsDAV_iCollection, jsDAV_iProperties, jsDAV_DAVACL_iACL);
    ...
    this.getChild = function(name, cbgetchild) {
        var self = this;
        self.caldavBackend.getCalendarObject(self.calendarInfo.id, name, function(err, obj) {
            if(err) return cbgetchild(err);
            if(!obj) return cbgetchild(new Exc.jsDAV_Exception_FileNotFound('Calendar object not found'));

            cbgetchild(null, new jsDAV_CalDAV_CalendarObject(self.caldavBackend, self.calendarInfo, obj));
        });
    }
    ...
    this.getChildren = function(callback) {
        var self = this;
        self.caldavBackend.getCalendarObjects(self.calendarInfo.id, function(err, objs) {
            if(err) return callback(err);

            var children = [];
            for(var i=0; i<objs.length; ++i)
                children.push(new jsDAV_CalDAV_CalendarObject(self.caldavBackend, self.calendarInfo, objs[i]));
     ...
     * @return void
     */
    this.createFile = function(name, calendarData, type, callback) {
