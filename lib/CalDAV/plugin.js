 ...
 */
module.exports = jsDAV_Plugin.extend({
    /**
     * Plugin name
     *
     * @var String
    ...
    getHTTPMethods: function() {
        // The MKCALENDAR is only available on unmapped uris, whose
        ...
        Async.list(hrefElems)
            ...
            .end(function(err) {
                ...
                self.handler.httpResponse.end(
                    self.handler.generateMultiStatus(propertyList, prefer["return-minimal"])
                );
            });
    },
    ...
    validateICal: function(data) {
        ...
        try {
            ...
            throw new Exc.UnsupportedMediaType("This resource only supports valid vcalendar " +
                                               "data. Parse error: " + ex.message);
        }

        if (vobj.name !== "VCALENDAR")
            throw new Exc.UnsupportedMediaType("This collection can only support vcalendar " +
                                               "objects.");
    },
         ...
         function afterCandidates(candidateNodes) {
             ...
             .end(function(err) {
                 ...
                 Async.list(validNodes).each(function(validNode, next) {
                     ...
                     self.handler.getPropertiesForPath(
                         href, query.requestedProperties, 0, function(err, props) {
                             if (err)
                                 return next(err);
                 ...
                 .end(function(err) {
                     ...
                     self.handler.httpResponse.end(
                         self.handler.generateMultiStatus(result, prefer["return-minimal"]));
                 });
             });
         }
     }
    ...
    //                 results.push(
    //                     this.validateParamFilters(
    //                         vProperties, filter["param-filters"], filter.test));
    ...
    //                 results.push(
    //                     this.validateTextMatches(texts, filter["text-matches"], filter.test));
    //             }
    ...
    //                 // text-match filter.
    //                 success = Util.textMatch(
    //                     vProperty.get(filter.name).value,
    //                     filter["text-match"].value, filter["text-match"]["match-type"]);
    //                 if (success)
    //                     break;
    //             }
    ...
    //     this.handler.createCollection(
    //         uri + "/" + postVars.name, resourceType, properties, function(err) {
    //             if (err)
    //                 return e.next(err);
    //             e.stop();
    //         }
