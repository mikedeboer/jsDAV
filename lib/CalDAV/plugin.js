...
function jsDAV_CalDAV_Plugin(handler) {
    ...
    this.handler.addEventListener("beforeWriteContent", this.beforeWriteContent.bind(this));
...
(function() {
    ...
    this.getSupportedReportSet = function(uri, callback) {
        this.handler.getNodeForPath(uri, function(err, node) {
            ...
            if(node.hasFeature(jsDAV.__ICALENDAR__) || node.hasFeature(jsDAV.__ICALENDAROBJECT__)) {
...
//                break;
    ...
    this.beforeWriteContent = function(e, path, node, data) {
        if(!node.hasFeature(jsDAV.__ICALENDAROBJECT__))
            return e.next();

        this.validateICalendar(e, data);
    }

    this.beforeCreateFile = function(e, path, data, parentNode) {
        if(!parentNode.hasFeature(jsDAV.__ICALENDAR__))
            return e.next();

        this.validateICalendar(e, data);
    }

    this.validateICalendar = function(e, data) {
        // NB: data should *always* be a buffer object, making this redundant
        if(data instanceof Buffer)
            data = data.toString('utf8');

        try {
            var vobj = icalendar.parse_calendar(data);
            // TODO: Perform further validation?
        }
        catch (err) {
            return e.next(new Exc.jsDAV_Exception_UnsupportedMediaType(
                'This resource only supports valid iCalendar 2.0 data. Parse error: '+err.toString()));
        }
        ...
        self.handler.getPropertiesForPath(self.handler.getRequestUri(), requestedProperties,
            self.handler.getHTTPDepth(1), function(err, candidateNodes) {
                ...
                for(var path in candidateNodes) {
                    ...
                    var vObject = icalendar.parse_calendar(node['200']['{'+NS_CALDAV+'}calendar-data']);
