...
(function() {
     ...
     */
    this.NS_CALDAV = 'urn:ietf:params:xml:ns:caldav';
    
    /**
     * This is the namespace for the proprietary calendarserver extensions
     */
    this.NS_CALENDARSERVER = 'http://calendarserver.org/ns/';
        ...
        Async.list(propertyNames)
            .each(function(prop, cbnextprop) {
                if(node.hasFeature(jsDAV.__IPRINCIPAL__)) {
                    switch(prop) {
                        case "{"+self.NS_CALDAV+"}calendar-home-set":
                            var principalId = node.getName();
                            var calendarHomePath = self.CALENDAR_ROOT+"/"+principalId+"/";
                            newProperties["200"][prop] = new jsDAV_Property_Href(calendarHomePath);
                            break;

                        case "{"+self.NS_CALDAV+"}calendar-user-address-set":
...
exports.jsDAV_CalDAV_Plugin = jsDAV_CalDAV_Plugin;
