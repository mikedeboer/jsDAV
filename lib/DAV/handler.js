            ...
            _self.server.tree.getNodeForPath(uri, function(err, node) {
                if (!Util.empty(err)) {
                    if (err instanceof Exc.jsDAV_Exception_FileNotFound) {
                        ...
                        _self.createFile(uri, body, "binary", function(err) {
                            if (!Util.empty(err))
                                return _self.handleError(err);
                ...
                Async.list(nodesPath)
                    ...
                    .each(function(myPath, cbnextpfp) {
                         ...
                         if (node.hasFeature(jsDAV.__IPROPERTIES__))
                             var getNodeProperties = node.getProperties;
                         else
                             var getNodeProperties = function(names, callback){ callback(null, {}) };

			 getNodeProperties.call(node, propertyNames, function(err, nodeProps){
                             if(err){
				if(!Array.isArray(err)){
				    err = propertyNames;
				}
				err.forEach(function(prop){
				    newProperties["404"][prop] = nodeProps[prop];
				    delete nodeProps[prop];
				});
			     }
			     newProperties["200"] = nodeProps;
                             if (allProperties) {
