...
(function() {
    ...
    this.invoke = function() {
        ...
        if (this.server.emit("beforeMethod", method))
            return;
        ...
        if (internalMethods[method]) {
            ...
            if (this.server.emit("unknownMethod", method)) {
                // Unsupported method
    ...
    this.httpDelete = function() {
        ...
        this.server.tree.getNodeForPath(uri, function(err, node) {
            ...
            if (_self.server.emit("beforeUnbind", uri))
                return;
    ...
    this.httpPut = function() {
        ...
        if (isStream) {
            ...
            var file   = this.server.tmpDir + "/" + Util.uuid(),
            ...
            _self.server.tree.getNodeForPath(uri, function(err, node) {
                if (!Util.empty(err)) {
                    ...
                    _self.checkPreconditions(false, function(err, redirected) {
                        ...
                        if (_self.server.emit("beforeWriteContent", uri)) {
                            cleanup();
                            return false;
                        }

                        node.put(body, type, function(err) {
                            cleanup();
                            if (!Util.empty(err))
                                return _self.handleError(err);
    ...
    this.httpMove = function() {
        ...
        this.getCopyAndMoveInfo(function(err, moveInfo) {
            ...
            if (moveInfo["destinationExists"]) {
                if (_self.server.emit("beforeUnbind", moveInfo["destination"]))
                    return false;
            ...
            function afterDelete() {
                if (_self.server.emit("beforeUnbind", moveInfo["source"])
                 || _self.server.emit("beforeBind",   moveInfo["destination"]))
                    return false;
                _self.server.tree.move(moveInfo["source"], moveInfo["destination"], function(err) {
                    if (!Util.empty(err))
                        return _self.handleError(err);
                    _self.server.emit("afterBind", moveInfo["destination"]);
    ...
    this.httpCopy = function() {
        ...
        this.getCopyAndMoveInfo(function(err, copyInfo) {
            ...
            if (copyInfo["destinationExists"]) {
                if (_self.server.emit("beforeUnbind", copyInfo["destination"]))
                    return false;
            ...
            function afterDelete() {
                if (_self.server.emit("beforeBind", copyInfo["destination"]))
                    return false;
                _self.server.tree.copy(copyInfo["source"], copyInfo["destination"], function(err) {
                    if (!Util.empty(err))
                        return _self.handleError(err);
                    _self.server.emit("afterBind", copyInfo["destination"]);
    ...
    this.httpReport = function() {
        ...
        req.addListener("end",  function() {
            Util.loadDOMDocument(data, function(err, dom) {
                ...
                if (_self.server.emit("report", reportName, dom)) {
    ...
    this.calculateUri = function(uri) {
        ...
        if (uri.indexOf(this.server.baseUri) === 0) {
            Sys.debug("INFO: returning uri: " + Util.trim(unescape(uri.substr(this.server.baseUri.length)), "/"));
            return Util.trim(unescape(uri.substr(this.server.baseUri.length)), "/");
        }
        ...
        this.server.tree.getNodeForPath(path, function(err, parentNode) {
            ...
            function afterGetChildren(nodes) {
                ...
                function afterGetProperty(rprop, rpath, remRT, newProps, cbnext) {
                    ...
                    _self.server.emit("afterGetProperties", [rpath, newProps]);
    ...
    this.parseProppatchRequest = function(body, cbproppatchreq) {
        ...
        Util.loadDOMDocument(body, function(err, dom) {
            ...
            for (var child in firstChild) {
                if (!(child instanceof Util.Element)) continue;
    ...
    this.createFile = function(uri, data, type, cbcreatefile) {
        ...
        if (this.server.emit("beforeBind", uri)
         || this.server.emit("beforeCreateFile", uri, data))
            return cbcreatefile();
    ...
    this.createCollection = function(uri, resourceType, properties, cbcreatecoll) {
        ...
        this.server.tree.getNodeForPath(parentUri, function(err, parent) {
            ...
            parent.getChild(newName, function(err, ch) {
                ...
                if (_self.server.emit("beforeBind", uri))
                    return cbcreatecoll();
