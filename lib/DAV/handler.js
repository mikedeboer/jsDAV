...
(function() {
    ...
    this.httpPut = function() {
        ...
        this.getRequestBody("binary", function(err, body, cleanup) {
            console.log("Body we got here:", body.toString());
            if (!Util.empty(err))
                return _self.handleError(err);
                ...
                if (!Util.empty(err)) {
                    ...
                    _self.checkPreconditions(false, function(err, redirected) {
                        ...
                        _self.dispatchEvent("beforeWriteContent", uri, function(stop) {
                            if (stop === true)
                                return cleanup();
                            console.log("WRITING TO FILE: ", node.path, typeof body, body.toString());
...
console.log("getting ENC:", enc);
        ...
        console.log("READING STREAM?? ", isStream, req.headers["content-type"]);

        if (!this.$getBodyStack)
            this.$getBodyStack = [];
        this.$getBodyStack.push([enc, cbreqbody]);
        if (this.$reading) {
            console.log("escaping call...", this.$reading);
            return;
        }
        ...
        if (isStream) {
            ...
            req.on("data", function(data) {
                ...
                console.log("pushing data on the stack: ", data.length);
                stream.push(data);
            });
            ...
            req.on("end", function() {
                clearTimeout(timeout);
                _self.$reading = false;
                console.log("STREAM CLOSED: ", stream.length);
        ...
        function readDone(err, data, cleanup) {
            ...
            _self.$getBodyStack.forEach(function(args) {
                enc = args[0];
                cbreqbody = args[1];
                console.log("getting content with encoding: ", enc);
