...
(function() {
    ...
    this.copy = function(source, destination, cbfscopy) {
		this.eventHandler.copyPath(source, destination, function (err) {
			if (err) {
				if (typeof(err) == "string") {
					return cbfscopy(new Exc.jsDAV_Exception_FileNotFound(err));
				} else {
					return err;
				}
			}

			return cbfscopy(null);
		});
    };
    ...
    this.move = function(source, destination, cbfsmove) {
		this.eventHandler.movePath(source, destination, function (err) {
			if (err) {
				if (typeof(err) == "string") {
					return cbfsmove(new Exc.jsDAV_Exception_FileNotFound(err));
				} else {
					return err;
				}
			}

			return cbfsmove(null);
		});
    };
