...
jsDAV_Codesearch_Plugin.SED_CMD = "sed";
...
(function() {
    ...
    this.doCodesearch = function(node, options, cbsearch) {
        ...
        if (!Util.empty(options.pattern)) {
            ...
            if (patterns > 0) { // could be that pattern == *, *.*
        ...
        if (Util.isTrue(options.replaceAll) && !Util.empty(options.replacement)) {
            var replacementValueSanitized = options.replacement.trim().replace(/([\[\]\{\}])/g, "\\$1");
            var sedReplace = "'s/" + options.query + "/" + replacementValueSanitized + "/g'";
            // all right, let's talk about this. grep returns filename:linenumber:query; pipe this into sed
            // and trim after the first ":"; for printing to STDOUT and to support parseSearchResult(),
            // take the grep result (FILE) and perform the same sed
            cmd += " | while read FILE ; do " + jsDAV_Codesearch_Plugin.SED_CMD + " -i '' " + sedReplace 
            + " ${FILE%%:*}; echo $FILE | " + jsDAV_Codesearch_Plugin.SED_CMD + " " + sedReplace + "; done;";
            /* Note that this does not work for sed, awk, or perhaps even gawk fully. sed and awk don't have shortened
            operator bindings (e.g., \w); gawk does, but its group matching is different (instead of $1 it uses \1) */
        }
    ...
    this.parseSearchResult = function(res, basePath, options) {
        ...
        var query = Util.isTrue(options.replaceAll) ? Util.escapeXml(options.replacement) : Util.escapeXml(options.query);
  
        aXml.push('><d:querydetail query="' + query + '" />');
        if (options.killed)
