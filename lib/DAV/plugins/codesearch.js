...
(function() {
    ...
    this.doCodesearch = function(node, options, cbsearch) {
        ...
        if (!Util.isTrue(options.replaceAll) && !Util.isTrue(options.regexp)) {
            ...
            for (var q in splitQuery) {
                splitQuery[q] = Util.escapeShell(Util.grepEscapeRegExp(splitQuery[q])); 
            }
        ...
        if (Util.isTrue(options.replaceAll) && !Util.empty(options.replacement)) {
            cmd += " -l | xargs " + jsDAV_Codesearch_Plugin.PERL_CMD + " -p0777i -e 'print STDOUT \"$ARGV:$.:$_\" if s/" + options.query + "/" + options.replacement + "/mg" + ( !Util.isTrue(options.casesensitive) ? "i" : "" ) + ";'"
        }
    ...
    this.parseSearchResult = function(res, basePath, options) {
        ...
        aXml.push('><d:querydetail query="' + options.query + '" replacement="' + options.replacement + '"/>');
        if (options.killed)
