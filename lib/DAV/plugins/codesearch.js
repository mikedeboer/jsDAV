...
(function() {
    ...
    this.doCodesearch = function(node, options, cbsearch) {
        console.log("Code Search", options, Util.escapeRegExp(options.query))
        ...
        cmd += " --exclude=*{" + jsDAV_Codesearch_Plugin.PATTERN_EDIR + "}*"
            +  " --include=" + include + " \"" + Util.escapeShell(options.regexp
                ? options.query
                : Util.escapeRegExp(options.query)) + "\" \"" + Util.escapeShell(node.path) + "\"";
        if (jsDAV.debugMode)
