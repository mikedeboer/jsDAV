...
(function() {
    ...
    this.parseSearchResult = function(res, basePath, options) {
        ...
        for (; i < l; ++i) {
            ...
            if (parts.length < 3) continue; // make sure all the grep pieces are there
            ++count;
            
            parts[0] = encodeURI(options.uri + Util.rtrim(parts[0].replace(basePath, "")), "/");
            ...
            parts[2] = truncate(parts[2], options);
        ...
        return Util.escapeXml(options.query) + ":" + Util.escapeXml(options.replacement) + ":" + optionsDesc + aLines.join("\n") + count + ":" + filecount;
    };
