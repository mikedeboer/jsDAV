...
(function() {
    ...
    this.httpReportHandler = function(e, reportName, dom) {
        ...
        this.handler.server.tree.getNodeForPath(uri, function(err, node) {
            ...
            _self.doCodesearch(node, options, function(err, sResults) {
                //if (err)
                //    return e.stop(err);
                _self.handler.httpResponse.writeHead(207, {"Content-Type":"text/html; charset=utf-8"});
    ...
    this.parseSearchResult = function(res, basePath, options) {
        var namespace, prefix, lastFile, parts,
        ...
        for (; i < l; ++i) {
            ...
            if (parts[0] !== lastFile) {
                ...
                aXml.push('<d:response path="' + encodeURI(options.uri
                    + Util.rtrim(parts[0].replace(basePath, "")), "/") + '">');
                lastFile = parts[0];
            }
            aXml.push('<d:excerpt line="', parts[1], '"><![CDATA[', parts[2],
                ']]></d:excerpt>');
        }
        if (count > 0)
            aXml.push('</d:response>');
