...
(function() {
    ...
    this.init = function(realm, req) {
        ...
        digestParts = parseDigest(digest);
    };
     ...
     * @return bool 
     */
    function validateA1(handler, newA1, cbvalida1) {
        A1 = newA1;
        validate(handler, cbvalida1);
    }
     ...
     * @return bool 
     */
    function validatePassword(handler, password, cbvalidpass) {
        A1 = Util.md5(digestParts["username"] + ":" + this.realm + ":" + password);
        validate(handler, cbvalidpass);
    }
     ...
     * @return bool 
     */
    function validate(handler, cbvalidate) {
        var req   = handler.httpRequest,
            A2    = req.method + ":" + digestParts["uri"];

        if (digestParts["qop"] == "auth-int") {
        ...
        function afterBody() {
            ...
            validResponse = Util.md5(A1 + ":" + digestParts["nonce"] + ":" 
                + digestParts["nc"] + ":" + digestParts["cnonce"] + ":" 
                + digestParts["qop"] + ":" + A2); 
    
            cbvalidate(digestParts["response"] == validResponse);
        }
    }
    ...
    this.authenticate = function(handler, realm, cbauth) {
        ...
        this.getDigestHash(realm, username, function(err, hash) {
            ...
            validateA1(handler, hash, function(isValid) {
                if (!isValid)
