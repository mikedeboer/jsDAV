            ...
            switch (propName) {
                case "{DAV:}supportedlock" :
                    if (self.locksBackend) {
                        ...
                        self.handler.tree.getNodeForPath(path, function(err, node) {
                            if (err)
    ...
    this.getLocks = function(uri, returnChildLocks, cbgetlocks) {
        ...
        Async.list(uri.split("/")).each(function(uriPart, next) {
            if (currentPath)
                currentPath += "/";
            currentPath += uriPart;
            self.handler.tree.getNodeForPath(currentPath, function(err, node) {
                if (err)
                    return next((err instanceof Exc.jsDAV_Exception_FileNotFound) ? null : err);
    ...
    this.httpLock = function(e) {
        ...
        this.validateLock(uri, false, function(err, isValid, lastLock) {
            ...
            self.handler.getRequestBody("utf8", function(err, body) {
                ...
                self.handler.tree.getNodeForPath(uri, function(err, node) {
                    if (err) {
