...
function jsDAV_Locks_Plugin(locksBackend) {
...
(function() {
    this.initialize = function(server) {
        ...
        server.on("beforeMethod", array(this,"beforeMethod"),50);
    ...
    this.getHTTPMethods = function(uri) {
        if (this.locksBackend ||
            this.handler.tree.getNodeForPath(uri) instanceof jsDAV_iLocks) {
            return ["LOCK","UNLOCK"];
        }
        return [];
    };
    ...
    this.httpLock = function() {
        ...
        try {
            ...
            this.handler.emit("beforeWriteContent", [uri]);
        }
        catch (ex) {
            if (ex instanceof Exc.jsDAV_Exception_FileNotFound) {
    ...
    this.unlockNode = function(uri, lockInfo) {
        ...
        try {
            ...
            // In case the node didn"t exist, this could be a lock-null request
            if (!(ex instanceof jsDAV_Exception_FileNotFound))
                throw ex;
        }
    ...
    this.validateLock = function(urls, lastLock) {
        ...
        var url, locks, condition, conditionUri, conditionToken, etagValid, lockValid,
            uri, node, lockToken, j, l2, k, l3, lockIndex, l4,
