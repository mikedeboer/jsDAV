...
(function() {
     ...
     * @return bool
     */
    this.afterGetProperties = function(e, path, newProperties) {
        var _self = this;
        Async.list(Object.keys(newProperties["404"])).each(function(propName, next) {
            ...
            switch (propName) {
                case "{DAV:}supportedlock" :
                    if (_self.locksBackend) {
                        val = true;
                        afterGetNode();
                    }
                    else {
                        _self.handler.server.tree.getNodeForPath(path, function(err, node) {
                            if (err)
                ...
                case "{DAV:}lockdiscovery" :
                    _self.getLocks(path, false, function(err, locks) {
                        if (err)
                            return next(err);
                        newProperties["200"][propName] = new jsDAV_Property_LockDiscovery(locks);
    ...
    this.beforeMethod = function(e, method) {
        ...
        console.log("GETTING INTO LOCK HANDLER", method);
        switch (method) {
            case "DELETE" :
            case "MKCOL" :
            case "PROPPATCH" :
            case "PUT" :
                this.validateLock(null, false, function(err, isValid, lastLock) {
                    console.log("EXIT LOCK HANDLER", err, isValid);
    ...
    this.getLocks = function(uri, returnChildLocks, cbgetlocks) {
        ...
        Async.list(uri.split("/")).each(function(uriPart, next) {
            if (currentPath)
                currentPath += "/";
            currentPath += uriPart;
            _self.handler.server.tree.getNodeForPath(currentPath, function(err, node) {
                if (err)
                    return next((err instanceof Exc.jsDAV_Exception_FileNotFound) ? null : err);
                if (node.hasFeature(jsDAV.__ILOCKABLE__)) {
        ...
        .end(function(err) {
            ...
            if (_self.locksBackend) {
                _self.locksBackend.getLocks(uri, returnChildLocks, function(err, locks) {
    ...
    this.httpLock = function(e) {
        ...
        this.validateLock(uri, false, function(err, isValid, lastLock) {
            ...
            _self.handler.getRequestBody("utf8", function(err, body) {
                if (err)
                    return e.next(err);
                ...
                if (body) {
                    ...
                    lockInfo       = _self.parseLockRequest(body);
                    lockInfo.depth = _self.handler.getHTTPDepth();
                    lockInfo.uri   = uri;
                ...
                try {
                    timeout = _self.getTimeoutHeader();
                }
                catch (ex) {
                    return e.next(ex);
                }
                ...
                _self.handler.server.tree.getNodeForPath(uri, function(err, node) {
                    if (err) {
                        if (ex instanceof Exc.jsDAV_Exception_FileNotFound) {
                            // It didn't, lets create it
                            _self.handler.createFile(uri, new Buffer(0), "utf8", function(err) {
                                if (err)
                                    return e.next(err);
                                newFile = true;
                                afterNode();
                    ...
                    function afterNode() {
                        ...
                        _self.handler.dispatchEvent("beforeWriteContent", [uri]);

                        _self.lockNode(uri, lockInfo);
                
                        _self.handler.httpResponse.writeHead(newFile ? 201 : 200, {
                            "Content-Type": "application/xml; charset=utf-8",
                            "Lock-Token": "<opaquelocktoken:" + lockInfo.token + ">"
                        });
                        _self.handler.httpResponse.end(_self.generateLockResponse(lockInfo));
                    }
                });
            });
        });
    ...
    this.httpUnlock = function(e) {
        ...
        this.getLocks(uri, false, function(err, locks) {
            ...
            if (found) {
                _self.unlockNode(uri, lock, function(err) {
                    if (err)
                        return e.next(err);
                    _self.handler.httpResponse.writeHead(204, {"Content-Length": "0"});
                    _self.handler.httpResponse.end();
                    e.stop();
                });
            }
            else {
     ...
     * @return void
     */
    this.lockNode = function(uri, cblock) {
        var _self = this;
        this.handler.dispatchEvent("beforeLock", [uri], function(stop, lockInfo) {
            if (stop === true)
            ...
            _self.handler.server.tree.getNodeForPath(uri, function(err, node) {
                ...
                if (node.hasFeature(jsDAV.__ILOCKABLE__))
                    return node.lock(cblock);
                
                if (_self.locksBackend)
                    return _self.locksBackend.lock(uri, cblock);
     ...
     * @return void
     */
    this.unlockNode = function(uri, cbunlock) {
        var _self = this;
        this.handler.dispatchEvent("beforeUnlock", [uri], function(stop, lockInfo) {
            if (stop === true)
            ...
            _self.handler.server.tree.getNodeForPath(uri, function(err, node) {
                ...
                if (node.hasFeature(jsDAV.__ILOCKABLE__))
                    return node.unlock(cbunlock);
                
                if (_self.locksBackend)
                    return _self.locksBackend.unlock(uri, cbunlock);
    ...
    this.generateLockResponse = function(lockInfo) {
        var lockObj = new jsDAV_Property_LockDiscovery([lockInfo], true);
     ...
     * @return bool
     */
    this.validateLock = function(urls, checkChildLocks, cbvalidate) {
        urls     = urls || null;
        lastLock = null;
        if (urls === null) {
        ...
        else if (!Array.isArray(urls.length)) {
        ...
        Async.list(urls).each(function(url, next) {
            _self.getLocks(url, false, function(err, aLocks) {
                if (err)
                    return next(err);
                ...
                Async.list(conditions).each(function(condition, next2) {
                    conditionUri = condition["uri"] ? _self.handler.calculateUri(condition["uri"]) : "";
                    ...
                    Async.list(condition["tokens"]).each(function(conditionToken, next3) {
                        ...
                        if (conditionToken[2]) {
                            var uri = conditionUri ? conditionUri : _self.handler.getRequestUri();
                            _self.handler.server.tree.getNodeForPath(uri, function(err, node) {
                                if (err)
    ...
    this.parseLockRequest = function(body) {
        var lockInfo = new jsDAV_Locks_LockInfo();

        var m = xml.match(/<d\:owner>([^<]*)<\/d\:owner>/mi);
        lockInfo.owner = m && m[1] ? Util.trim(m[1]) : null;

        var id = md5(Date.now() + "somethingrandom");
        var lockToken = "44445502-" + id.substr(0, 4) + "-" + id.substr(4, 4) 
                      + "-" + id.substr(8, 4) + "-" + idsubstr(12, 12);
