...
//        "icons/calendar": jsCalDAV_iCalendar,
    ...
    httpPOSTHandler: function(e, method, uri) {
        ...
        form.on("end", function() {
            ...
            self.handler.dispatchEvent("onBrowserPostAction", uri, postVars.jsdavAction, postVars, function(stop) {
                if (!stop) {
                    var name = Util.trim(postVars.name);
                    switch (postVars.jsdavAction) {
                        case "mkcol":
                            if (name) {
                                // Using basename() because we won't allow slashes
                                var folderName = Util.splitPath(name)[1];
                                self.handler.createDirectory(uri + "/" + folderName, finish);
                            }
                            break;
                        case "put":
                            var file = postVars.tmp_file;
                            if (!file)
                                break;
                            var newName = name ? name : Util.splitPath(file)[1];
                            // Making sure we only have a 'basename' component
                            newName = Util.splitPath(newName)[1];
                            Fs.readFile(file, function(err, buf) {
                                if (err)
                                    return e.next(err);
                                
                                self.handler.createFile(uri + "/" + newName, buf, "binary", finish);
                            });
                            break;
    ...
    <tr><td colspan=\"5\"><hr /></td></tr>\n";
        ...
        self.handler.getPropertiesForPath(path, [
            ...
            self.handler.getNodeForPath(path, function(err, parent) {
                ...
                var file, icon, name, type, size, lastmodified, fullPath, i, l, node;
