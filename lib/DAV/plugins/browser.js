...
(function() {
    ...
    this.httpGetInterceptor = function(e, method) {
        ...
        var self = this;
        this.handler.tree.getNodeForPath(uri, function(err, node) {
            if (err || node.hasFeature(jsDAV.__IFILE__))
                return e.next();

            self.generateDirectoryIndex(self.handler.getRequestUri(), function(err, sIndex) {
                if (err)
                    return e.next(err);
            ...
            switch(_POST["action"]) {
                ...
                case "put" :
                    ...
                    if (is_uploaded_file(file["tmp_name"])) {
                        parent = this.tree.getNodeForPath(trim(this.server.getRequestUri(),"/"));
