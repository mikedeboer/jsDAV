    ...
    this.initialize = function() {
        ...
        if (tree.$ready) {
            if (!tree.ftp.socket) {
                ...
                tree.$ready = false;
                //tree.ftp.connect();
                var options = tree.ftp.options;

                tree.ftp.destroy();
                tree.ftp = new Ftp(options);
            }
            else {
                return;
            }
        ...
        var onReady = function(oldInvoke, args) {
            if (!tree.$ready)
                console.info('> Ftp connection ready on: '+ tree.options.host);
