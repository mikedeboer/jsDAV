...
function Server(options) {
    ...
    else if (!options || !options.tree || !options.node) {
...
(function() {
    ...
    this.invoke = function() {
        ...
        if (internalMethods[method]) {
            this["http" + method.charAt(0) + method.substr(1)]();
        }
        else {
    ...
    this.httpOptions = function() {
        var methods = this.getAllowedMethods(this.getRequestUri()),
            headers = {
    ...
    this.httpGet = function() {
         ...
         * this method
         */
        var httpHeaders = this.getHTTPHeaders(uri),
            nodeSize    = null;
        ...
        if (!ignoreRangeHeader && nodeSize && range) {
            ...
            this.httpResponse.writeHead(206, httpHeaders);
            this.httpResponse.end(newStream);
        }
        else {
            if (nodeSize)
                httpHeaders["Content-Length"] = nodeSize;
            this.httpResponse.writeHead(200, httpHeaders);
    ...
    this.httpHead = function() {
        var node = this.tree.getNodeForPath(this.getRequestUri());
    ...
    this.httpPropfind = function() {
        // xml = new jsDAV_XMLReader(file_get_contents('php://input'));
        var requestedProperties = this.parsePropfindRequest(this.httpRequest.getBody(true)),
        ...
        this.httpResponse.end(this.generateMultiStatus(newProperties));
    };
    ...
    this.httpProppatch = function() {
        var newProperties = this.parseProppatchRequest(this.httpRequest.getBody(true)),
    ...
    this.httpPut = function() {
        ...
        if ((expected = this.httpRequest.headers("X-Expected-Entity-Length")) && expected > 0) {
            /**
        ...
        try {
            ...
            if (!this.emit("beforeWriteContent", this.getRequestUri()))
                return false;

            node.put(body);
            this.httpResponse.writeHead(200, {"Content-Length": "0"});
            this.httpResponse.end();
        }
        catch (e) {
    ...
    this.httpMove = function() {
        ...
        if (!this.emit("beforeUnbind", moveInfo["source"])
         || !this.emit("beforeBind",   moveInfo["destination"]))
            return false;
    ...
    this.httpCopy = function() {
        ...
        if (!this.emit("beforeBind", copyInfo["destination"]))
            return false;
     ...
     * @return {Array}
     */
    this.getAllowedMethods = function(uri) {
        var methods = [
            "OPTIONS",
            "GET",
            "HEAD",
     ...
     * @param {String} path
     */
    this.getHTTPHeaders = function(path) {
        var header, prop,
            propertyMap = {
            ...
            properties = this.getProperties(path, ["{DAV:}getcontenttype",
                "{DAV:}getcontentlength", "{DAV:}getlastmodified", "{DAV:}getetag"]),
            headers    = {};
        ...
        for (prop in propertyMap) {
            ...
            if (properties[prop]) {
                ...
                if (properties[prop].hasFeature(jsDAV.__PROPGETLASTMODIFIED__)) {
                    headers[header] = properties[prop].getTime().format(DateTime::RFC1123);
                }
                else
     ...
     */
    this.getProperties = function(path, propertyNames) {
        var result = this.getPropertiesForPath(path, propertyNames, 0);
        return result[0]["200"];
    };
     ...
     * @return {Array}
     */
    this.getPropertiesForPath = function(path, propertyNames, depth) {
        ...
        if (depth == 1 && parentNode.hasFEATURE(jsDAV.__ICOLLECTION__)) {
            for (cNodes = parentNode.getChildren(), i = 0, l = cNodes.length; i < l; ++i)
        ...
        for (myPath in nodes) {
            ...
            this.emit("afterGetProperties", [Util.trim(myPath, "/"), newProperties]);
    ...
    this.createCollection = function(uri, resourceType, properties) {
        ...
        try {
            ...
            if (!e.type != "jsDAV_Exception_MethodNotAllowed")
                throw e;
        }
    ...
    this.getCopyAndMoveInfo = function() {
        ...
        try {
            ...
            throw (e.type == "jsDAV_Exception_FileNotFound")
        ...
        try {
            ...
            if (e.type == "jsDAV_Exception_FileNotFound")
