...
function Server(options) {
    ...
    if (options.plugins) {
        ...
        for (var i = 0, l = allPlugins.length; i < l; ++i) {
            ...
            // they exist.
            if (options.plugins.indeOf(allPlugins[i]) == -1)
    ...
    if (typeof options.tree == "object" && options.tree.hasFeature(jsDAV.__TREE__)) {
        ...
        else if (options.type == "eventHandler") {
            var jsDAV_Tree_Handler = require("./tree/handler").jsDAV_Tree_Handler;
			var jsDAV_Locks_Backend_FS = require("./plugins/locks/fs");

			this.options.locksBackend = new jsDAV_Locks_Backend_FS(__dirname + "/temp");
            this.tree = new jsDAV_Tree_Handler(options);
        }
    }
