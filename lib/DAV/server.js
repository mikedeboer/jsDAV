...
(function() {
    ...
    this.httpOptions = function() {
        ...
        this.getAllowedMethods(uri, function(err, methods) {
            ...
            var headers = {
                    ...
                    "MS-Author-Via"   : "DAV",
                    "Accept-Ranges"   : "bytes",
                    "X-jsDAV-Version" : Server.VERSION,
                    "Content-Length"  : 0
                },
    ...
    this.httpGet = function() {
        ...
        this.checkPreconditions(true, function(err, redirected) {
            ...
            if (redirected)
                return;
            _self.tree.getNodeForPath(uri, 0, function(err, n) {
                if (err)
                    return _self.handleError(err);
                node = n;
                afterCheck();
        ...
        form.addListener("file", function(field, file) {
            ...
            _self.tree.getNodeForPath(uri, function(err, node) {
                ...
                _self.checkPreconditions(false, function(err, redirected) {
                    ...
                    node.exists(function(exists) {
                        if (!exists)
                            return _self.createFile(uri, file.path, onAfterWrite);
                        node.put(file.path, function(err) {
                            if (err)
                                return _self.handleError(err);
                            onAfterWrite();
                        });
    ...
    this.httpMkcol = function() {
        ...
        req.addListener("end",  function() {
            if (requestBody) {
                var contentType = _self.httpRequest.headers("content-type");
     ...
     * @return bool
     */
    this.checkPreconditions = function(handleAsGET, callback) {
        ...
        if (ifMatch = this.httpRequest.headers["if-match"]) {
            ...
            this.tree.getNodeForPath(uri, function(err, n) {
                if (err) {
                    return callback(new Exc.jsDAV_Exception_PreconditionFailed(
                        ...
                        if (nodeExists) {
                            ...
                            if (ifNoneMatch === "*" || ((etag = node.getETag()) && etag === ifNoneMatch)) {
                                if (handleAsGET) {
                                    ...
                                    _self.httpResponse.end();
                                    callback(null, true);
                                    // @todo call callback() differently here?
                                }
                                else {
                                    callback(new Exc.jsDAV_Exception_PreconditionFailed(
                        ...
                        if (ifUnmodifiedSince = _self.httpRequest.headers["if-unmodified-since"]) {
                            ...
                            function finale() {
                                ...
                                if (lastMod) {
                                    lastMod = new Date("@" + lastMod);
                                    if (lastMod > date) {
                                        return callback(Exc.jsDAV_Exception_PreconditionFailed(
    ...
    this.getPropertiesForPath = function(path, propertyNames, depth, callback) {
        ...
        this.tree.getNodeForPath(path, function(err, parentNode) {
            ...
            function afterGetChildren() {
                ...
                var keys = Util.hashKeys(nodes);
                Async.list(0, keys.length - 1)
                     .each(function(i, next) {
                         myPath = nodes[keys[i]];
                        ...
                        Async.list(propertyNames)
                             .each(function(prop, next2) {
                                 if (newProperties["200"][prop])
                                     return next2();
                                 ...
                                 if (prop == "{DAV:}getlastmodified") {
                                     ...
                                     if (node.hasFeature(jsDAV.__IQUOTA__)) {
                                         node.getQuotaInfo(function(err, quotaInfo) {
     ...
     * @return {Array}
     */
    this.parsePropfindRequest = function(body, callback) {
        ...
        var oXml = Util.loadDOMDocument(body, function(err, oXml) {
            ...
            if (err)
                return callback(err);
            callback(null, Util.hashKeys(Util.parseProperties(oXml.propfind || oXml)));
        });
    };
     ...
     */
    this.parseProppatchRequest = function(body, callback) {
     ...
     * @return {void}
     */
    this.createFile = function(uri, data, callback) {
     ...
     * @return {void}
     */
    this.createCollection = function(uri, resourceType, properties, callback) {
        ...
        if (resourceType.indexOf("{DAV:}collection") == -1) {
            return callback(new Exc.jsDAV_Exception_InvalidResourceType(
        ...
        this.tree.getNodeForPath(parentUri, function(err, parent) {
            if (err)
                return callback(new Exc.jsDAV_Exception_Conflict("Parent node does not exist"));

            // Making sure the parent is a collection
            if (!parent.hasFeature(jsDAV.__ICOLLECTION__))
                return callback(new Exc.jsDAV_Exception_Conflict("Parent node is not a collection"));
            ...
            parent.getChild(newName, function(err, ch) {
                ...
                if (!ch) {
                    return callback(new Exc.jsDAV_Exception_MethodNotAllowed(
                ...
                if (parent.hasFeature(jsDAV.__IEXTCOLLECTION__)) {
                    parent.createExtendedCollection(newName, resourceType, properties, callback);
                }
                else {
     ...
     * @return {Object}
     */
    this.getCopyAndMoveInfo = function(callback) {
        ...
        if (!this.httpRequest.headers["destination"])
            return callback(new Exc.jsDAV_Exception_BadRequest("The destination header was not supplied"));
        ...
        this.tree.getNodeForPath(destinationDir, function(err, destinationParent) {
            if (err) {
                ...
                return callback(err.type == "jsDAV_Exception_FileNotFound"
                    ? new Exc.jsDAV_Exception_Conflict("The destination node is not found")
                    : err);
            }
            if (!destinationParent.hasFeature(jsDAV.__ICOLLECTION__)) {
                return callback(new Exc.jsDAV_Exception_UnsupportedMediaType(
                    "The destination node is not a collection")
                );
            }

            this.tree.getNodeForPath(destination, function(err, destinationNode) {
