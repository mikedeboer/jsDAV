...
(function() {
    ...
    this.put = function(data, type, cbftpput) {
        ...
        if (!Buffer.isBuffer(buffer))
            buffer = new Buffer(buffer, type || "binary");

        this.ftp.put(this.path, buffer, function(err) {
            if (err)
                return cbftpput(err);
    ...
    this["delete"] = function(cbftpfiledel) {
        var self = this;
        this.ftp.raw.dele(this.path, function(err){
            if (err)
                return cbftpfiledel(
                    new Exc.jsDAV_Exception_FileNotFound("File " + self.path
                        + " not found"));
