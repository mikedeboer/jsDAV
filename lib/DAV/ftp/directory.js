...
(function() {
    ...
    this.getChild = function(stat, cbftpgetchild) {
        ...
        var ftp = this.ftp;
        var path = Util.rtrim(Path.join(this.path, stat.name), "/");
        if (ftp.$cache[path])
        ...
        function getDAVObject(type /*int*/, path) {
            if (type === 1)
                ...
                return cbftpgetchild(
                    new jsDAV_Exception_UnsupportedMediaType(path));
        ...
        // it). After that we give back a DAV object of the tye of the target,
    ...
    this.realDelete = function(cbftpdel) {
        var self = this;
        this.ftp.raw.rmd(this.path, function(err) {
            if (err)
                return cbftpdel(err);
