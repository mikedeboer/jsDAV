...
(function() {
    ...
    this.get = function(cbfsfileget) {
		this.eventHandler.getFile(this.path, function (err, buff) {
			if (err) {
				if (typeof(err) == "string") {
					return cbfsfileget(new Exc.jsDAV_Exception_FileNotFound(err));
				} else {
    ...
    this["delete"] = function(cbfsfiledel) {
		this.eventHandler.deleteFile(this.path, function (err, buff) {
			if (err) {
				if (typeof(err) == "string") {
					return cbfsfiledel(new Exc.jsDAV_Exception_FileNotFound(err));
				} else {
					return cbfsfiledel(err);
				}
			}

			cbfsfiledel(null, buff);
		});
    };
    ...
    this.getSize = function(cbfsgetsize) {
		this.eventHandler.getFileSize(this.path, function (err, buff) {
			if (err) {
				if (typeof(err) == "string") {
					return cbfsgetsize(new Exc.jsDAV_Exception_FileNotFound(err));
				} else {
					return cbfsgetsize(err);
				}
			}

			cbfsgetsize(null, buff);
		});
    };
