...
(function() {
     ...
     * @return void
     */
    this.copy = function(sourcePath, destinationPath, callback) {
        this.getNodeForPath(sourcePath, function(err, sourceNode) {
            ...
            this.getNodeForPath(destinationDir, function(err, destinationParent) {
                if (err)
                    return callback(err);
                this.copyNode(sourceNode, destinationParent, destinationName, callback);
            });
        });
    };
     ...
     * @return int
     */
    this.move = function(sourcePath, destinationPath, callback) {
        ...
        if (sourceDir === destinationDir) {
            this.getNodeForPath(sourcePath, function(err, renameable) {
                if (err)
                    return callback(err);
                renameable.setName(destinationName, callback);
            });
        }
        else {
            var _self = this;
            this.copy(sourcePath, destinationPath, function(err) {
                if (err)
                    return callback(err);
                _self.getNodeForPath(sourcePath)["delete"](callback);
            });
        }
    };
     ...
     * @return void
     */
    this.copyNode = function(source, destinationParent, destinationName, callback) {
        if (!destinationName)
        ...
        if (source.hasFeature(jsDAV.__IFILE__)) {
            ...
            destinationParent.createDirectory(destinationName, function(err) {
                ...
                destinationParent.getChild(destinationName, function(err, destination) {
                    ...
                    for (; i < l && !error; ++i) {
                        child = c[i];
                        this.copyNode(child, destination, onError);
                    }
                    if (error)
        ...
        function afterCopy(destination) {
            if (source.hasFeature(jsDAV.__IPROPERTIES__) && destination.hasFeature(jsDAV.__IPROPERTIES__))
                destination.updateProperties(source.getProperties({}), callback);
            else
                callback();
        }
    };
