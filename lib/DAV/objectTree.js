        ...
        Async.range(i, l - 1)
             .each(function(pathPart, next) {
                ...
                if (!currentNode.hasFeature(jsDAV.__ICOLLECTION__))
                    return next(new Exc.jsDAV_Exception_FileNotFound("Could not find node at path: " + path));

                currentNode = currentNode.getChild(pathPart, function(err, crt) {
                    if (err)
                        return next(err);
                    currentNode = crt;
                });
