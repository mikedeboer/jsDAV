 ...
 * @return string
 */
exports.toClarkNotation = function(node_or_ns, name) {
    var ns, name;
    if (!node_or_ns)
        return null;
    if(node_or_ns instanceof Xml.Element) {
        ns = node_or_ns.namespace();
        if(ns) ns = ns.href();
        name = node_or_ns.name();
    }
    else {
        ns = node_or_ns;
    }
...
/**
 * Converts a string from {DAV:}element-name to a two element array.
 *
 * If the element namespace was not specified, the first element of the
 * array will be null.
 */
exports.fromClarkNotation = function(element) {
    var parts = element.match(/^\{([^\}]*)\}(.*)/);
    if(!parts || parts[1] === "")
        return [null, element];
    else
...
exports.loadDOMDocument = function(xml, callback) {
    ...
    var root = Xml.parseXmlString(xml);
    if(root instanceof Xml.Document)
        callback(null, root);
    else
        callback(new Exc.jsDAV_Exception_BadRequest("The request body had an invalid XML body."));
};
...
exports.parseProperties = function(parentNode, propertyMap) {
    ...
    var proplist = {};
    var propnodes = parentNode.find('xmlns:prop', 'DAV:');
    for(var i=0; i<propnodes.length; ++i) {
        var children = propnodes[i].childNodes();
        for(var j=0; j<children.length; ++j) {
            var propdata = children[j];

            if(propdata.type() != 'element') continue;

            var propname = exports.toClarkNotation(propdata);
            if(propertyMap[propname])
                proplist[propname] = propertyMap[propname].unserialize(propdata);
            else
                proplist[propname] = propdata.text();
        }
    }
...
(function() {
    ...
    this.dispatchEvent = function() {
        ...
        Async.list(listeners).each(function(listener, cbnext) {
            var timeout = null;
            var e = new exports.Event(eventName, function() {
                if(timeout) clearTimeout(timeout);
                cbnext.apply(null, arguments);
            });

            if (listener.$usetimeout > 0) {
                timeout = setTimeout(function() {
                    e.next("Event callback timeout: timeout reached, no "+
                        "callback fired within "+listener.$usetimeout+"ms");
                }, listener.$usetimeout);
            }
            ...
            if (jsDAV.debugMode && err && err !== true)
...
exports.Event = function(type, callback) {
    ...
    this.next = function(err) {
        if(this.done)
            throw new Error("Event callback fired twice!");
