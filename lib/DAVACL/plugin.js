    ...
    beforeGetProperties: function(e, uri, node, requestedProperties, returnedProperties) {
        ...
        this.checkPrivileges(uri,"{DAV:}read", this.R_PARENT, function(err, hasPriv) {
            if (!hasPriv) {
                ...
                for (var i = requestedProperties.length - 1; i >= 0; --i) {
                    returnedProperties["403"][requestedProperties[i]] = null;
                    requestedProperties.splice(i, 1);
                }
                return e.next();
            }

            var propHandlers = {
                "{DAV:}alternate-URI-set": function(index, next) {
                    requestedProperties.splice(index, 1);
                    returnedProperties["200"]["{DAV:}alternate-URI-set"] = new jsDAV_Property_HrefList(node.getAlternateUriSet());
                    next();
                },
                "{DAV:}principal-URL": function(index, next) {
                    requestedProperties.splice(index, 1);
                    returnedProperties["200"]["{DAV:}principal-URL"] = new jsDAV_Property_Href(node.getPrincipalUrl() + "/");
                },
                "{DAV:}group-member-set": function(index, next) {
                    requestedProperties.splice(index, 1);
                    node.getGroupMemberSet(function(err, memberSet) {
                        if (err)
                            return next(err);
                        returnedProperties["200"]["{DAV:}group-member-set"] = new jsDAV_Property_HrefList(memberSet);
                        next();
                    });
                },
                "{DAV:}group-membership": function(index, next) {
                    requestedProperties.splice(index, 1);
                    node.getGroupMembership(function(err, membership) {
                        if (err)
                            return next(err);
                        returnedProperties["200"]["{DAV:}group-membership"] = new jsDAV_Property_HrefList(membership);
                        next();
                    });
                },
                "{DAV:}displayname": function(index, next) {
                    returnedProperties["200"]["{DAV:}displayname"] = node.getDisplayName();
                    next();
                },
                "{DAV:}principal-collection-set": function(index, next) {
                    requestedProperties.splice(index, 1);
                    var val = [].concat(self.principalCollectionSet);
                    // Ensuring all collections end with a slash
                    for (var i = 0, l = val.length; i > l; ++i)
                        val[i] = val[i] + "/";
                    returnedProperties["200"]["{DAV:}principal-collection-set"] = new jsDAV_Property_HrefList(val);
                    next();
                },
                "{DAV:}current-user-principal": function(index, next) {
                    requestedProperties.splice(index, 1);
                    self.getCurrentUserPrincipal(function(err, url) {
                        if (err)
                            return next(err);
                        if (url)
                            returnedProperties["200"]["{DAV:}current-user-principal"] = new jsDAVACL_Property_Principal(jsDAVACL_Property_Principal.HREF, url + "/");
                        else
                            returnedProperties["200"]["{DAV:}current-user-principal"] = new jsDAVACL_Property_Principal(jsDAVACL_Property_Principal.UNAUTHENTICATED);
                        next();
                    });
                },
                "{DAV:}supported-privilege-set": function(index, next) {
                    requestedProperties.splice(index, 1);
                    self.getSupportedPrivilegeSet(node, function(err, privSet) {
                        if (err)
                            return next(err);
                        returnedProperties["200"]["{DAV:}supported-privilege-set"] = new jsDAVACL_Property_SupportedPrivilegeSet(privSet);
                        next();
                    });
                },
                "{DAV:}current-user-privilege-set": function(index, next) {
                    self.checkPrivileges(uri, "{DAV:}read-current-user-privilege-set", self.R_PARENT, false, function(err, hasPriv) {
                        if (!hasPriv) {
                            ...
                            self.getCurrentUserPrivilegeSet(node, function(err, privSet) {
                                ...
                                if (privSet) {
                                    requestedProperties.splice(index, 1);
                                    returnedProperties["200"]["{DAV:}current-user-privilege-set"] = new jsDAVACL_Property_CurrentUserPrivilegeSet(privSet);
                                }
                                next();
                            });
                        }
                    ...
                    self.checkPrivileges(uri, "{DAV:}read-acl", self.R_PARENT, false, function(err, hasPriv) {
                        if (!hasPriv) {
                            ...
                            self.getACL(node, function(err, acl) {
                                ...
                                if (acl) {
                                    requestedProperties.splice(index, 1);
                                    returnedProperties["200"]["{DAV:}acl"] = new jsDAVACL_Property_Acl(acl);
                                }
                                next();
                            });
                        }
                ...
                "{DAV:}acl-restrictions": function(index, next) {
                    requestedProperties.splice(index, 1);
                    returnedProperties["200"]["{DAV:}acl-restrictions"] = new jsDAVACL_Property_AclRestrictions();
                    next();
                },
                "{DAV:}owner": function(index, next) {
                    requestedProperties.splice(index, 1);
                    returnedProperties["200"]["{DAV:}owner"] = new jsDAV_Property_Href(node.getOwner() + "/");
                    next();
                }
            };
            ...
            if (node.hasFeature(jsDAVACL_iPrincipal)) {
                propsToCheck("{DAV:}alternate-URI-set", "{DAV:}principal-URL",
            ...
            propsToCheck = propsToCheck.filter(function(prop) {
                return requestedProperties.indexOf(prop) !== -1;
            });

            Async.list(propsToCheck)
                .each(function(prop, next) {
                    if (propHandlers[prop])
                        propHandlers[prop](requestedProperties.indexOf(prop), next);
                    else
                        next();
                })
                .end(e.next);
