        ...
        self.getCurrentUserPrivilegeSet(path, function(err, acl) {
            if(acl === null) {
                if(self.allowAccessToNodesWithoutACL)
                    ...
                    cbcheckprivs(new ACL_Exc.jsDAV_DAVACL_Exception_NeedPrivileges(uri, privileges), false);
            }


            console.log('WARNING: Not checking privileges!!');
            cbcheckprivs(null, true);
...
//            return true;
    ...
    this.getSupportedPrivilegeSet = function(node) {
        ...
        return this.getDefaultSupportedPrivilegeSet();
    }
     ...
     * @return array
     */
    this.getDefaultSupportedPrivilegeSet = function() {
        return {
            'privilege': '{DAV:}all',
            'abstract': true,
            'aggregates': [
        ...
        function traverse(priv, concrete) {
            ...
            if(priv.aggregates) {
                for(var i=0; i<priv.aggregates.length; ++i)
                    myPriv.aggregates.push(priv.aggregates[i]);
            }
