...
var jsCardDAV_Plugin = module.exports = jsDAV_Plugin.extend({
    ...
    validateParamFilters: function(vProperties, filters, test) {
        var filter, isDefined, j, success;
        var vLen = vProperties.length;
        for (var i = 0, l = filters.length; i < l; ++i) {
            filter = filters[i];
            isDefined = false;
            for (j = 0; j < vLen; ++j) {
                isDefined = !!vProperties[j][filter.name];
                if (isDefined)
                    break;
            }
            ...
            // If there's no text-match, we can just check for existence
                ...
                for (j = 0; j < vLen; ++j) {
                    ...
                    // text-match filter.
                    success = Util.textMatch(vProperties[j][filter.name].value, 
                        filter["text-match"].value, filter["text-match"].collation, 
    ...
    validateTextMatches: function(texts, filters, test) {
        var filter, success, j, haystack;
        var textsLen = texts.length;
        for (var i = 0, l = filters.length; i < l; ++i) {
            filter = filters[i];
            success = false;
            for (j = 0; j < textsLen; ++j) {
                haystack = texts[j];
                success = Util.textMatch(haystack, filter.value, filter.collation, filter["match-type"]);
     ...
     * jsDAV_Browser_Plugin. This allows us to generate an interface users
