...
(function() {
    ...
    this.getChild = function(name, cbgetchild) {
        ...
        this.principalBackend.getPrincipalByPath(this.prefix+"/"+name, function(err, principal) {
            // The iPhone expects the principal URL to be /principals/users/<username>
            //  Here is where we try to make it happy
            if((err || !principal) && name === 'users')
                return cbgetchild(null, self);
