...
var jsDAVACL_Backend_Mongo = module.exports = jsDAVACL_iBackend.extend({
    ...
    initialize: function(mongo, tableName, groupMembersTableName) {
    	...
    	console.log("initializing davacl backend...", this.tableName, this.groupMembersTableName);
    ...
    getPrincipalsByPrefix: function(prefixPath, callback) {
        ...
        this.mongo.keys(this.tableName + "/" + prefixPath + "*", function(err, res) {
            if (err)
                return callback(err);
    ...
    getPrincipalByPath: function(path, callback) {
	    ...
	    console.log("GET PRINCIPAL BY PATH!...", path);
        var self = this;
...
/*
        ...
        this.mongo.hmget.apply(this.mongo, command);
*/
    },
    ...
    getGroupMemberSet: function(principal, callback) {
        ...
        this.getPrincipalByPath(principal, function(err, principal) {
            ...
            self.mongo.zrange(self.groupMembersTableName + "/" + principal, 0, -1, function(err, res) {
                if (err)
                    return err;
    ...
    getGroupMemberShip: function(principal, callback) {
        ...
        this.getPrincipalByPath(principal, function(err, principal) {
            ...
            self.mongo.zrange(self.tableName + "/" + principal + "/" + self.groupMembersTableName, 0, -1, function(err, res) {
                if (err)
                    return callback(err);
    ...
    setGroupMemberSet: function(principal, members, callback) {
        ...
        this.mongo.zrange(this.groupMembersTableName + "/" + principal, 0, -1, function(err, res) {
            if (err)
                return callback(err);
            ...
            self.mongo.multi(commands).exec(function(err) {
                if (err)
                    return callback(err);
