...
var jsDAVACL_Backend_Postgre = module.exports = jsDAVACL_iBackend.extend({
    ...
    getPrincipalsByPrefix: function(prefixPath, callback) {
        var self = this;
        this.pg.query('SELECT * FROM ' + this.tableName + ' WHERE "uri" LIKE $1', [prefixPath + '%'], function(err, result) {
            if (err)
                return callback(err);
    ...
    getPrincipalByPath: function(path, callback) {
        var self = this;
        this.pg.query('SELECT * FROM ' + this.tableName + ' WHERE "uri" = $1', [path], function(err, result) {
            if (err)
                return callback(err);
    ...
    updatePrincipal: function(path, mutations, callback) {
        ...
        this.pg.query(
            'UPDATE ' + this.tableName + 
                ' SET ' + sets + ' WHERE "uri"= $' + prepareCounter, 
            values, 
            function(err, result) {
                if (err)
                    callback(err, false);
    ...
    getGroupMemberSet: function(principal, callback) {
        ...
        this.getPrincipalByPath(principal, function(err, principal) {
            ...
            self.pg.query('SELECT member FROM ' + self.groupMembersTableName + ' WHERE "group"=$1', [principal.uri], function(err, result) {
                if (err)
                    return callback(err);

                var out = [];
                for (var i = 0; i < result.rows.length; i++) {
                    out.push(result.rows[i].member);
                }
                callback(null, out);
            });
