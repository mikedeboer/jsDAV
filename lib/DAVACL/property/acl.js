...
var jsDAV_Principal = require("./principal");
...
var jsDAV_Acl = module.exports = jsDAV_Property.extend({
     ...
     * @return void
     */
    serialize: function(server, strXml) {
        var aXml = [];
        for (var i = 0, l = this.privileges.length; i < l; ++i)
            aXml.push(this.serializeAce(this.privileges[i], server));
    ...
    unserialize: function(dom) {
        ...
        for (var i = 0, l = xaces.length; i < l; ++i) {
            ...
            principal = new jsDAV_Principal().unserialize(principal[0]);

            switch (principal.getType()) {
                case jsDAV_Principal.HREF :
                    principal = principal.getHref();
                    break;
                case jsDAV_Principal.AUTHENTICATED :
                    principal = "{DAV:}authenticated";
                    break;
                case jsDAV_Principal.UNAUTHENTICATED :
                    principal = "{DAV:}unauthenticated";
                    break;
                case jsDAV_Principal.ALL :
                    principal = "{DAV:}all";
                    break;
            }
     ...
     * @return void
     */
    serializeAce: function(ace, server) {
        ...
        switch(ace.principal) {
            ...
            case "{DAV:}all" :
                ...
                break;
            default:
                aXml.push("<d:href>", (this.prefixBaseUrl ? server.getBaseUri() : "") + ace.principal + "/", "</d:href>");
        }
