...
FTP.prototype.copy = function(origpath, destpath, cb) {
    if (this._state !== 'authorized')
        return false;
...
FTP.prototype.readdir = function(path, cb) {
    var self = this;
    this.list(path, function(err, iter) {
        if (err)
            return cb(err);
        ...
        iter.on('error', function(e) { // Under normal circumstances this shouldn't happen.
            self.ftp.end();
            cb('Error during LIST(): ' + util.inspect(e));
        });
 ...
 */
var Stat = FTP.Stat = function(struct) {
 ...
 * @type  {void}
 */
FTP.prototype.stat = function(path, callback) {
    var _self = this,
        parts = path.split("/"),
        node = parts.pop(),
        root = parts.join("/");
    ...
    function afterPwd() {
        if (debug)
            debug('> STAT ' + root);
        _self.list(root, function(err, iter) {
            if (err)
                throw err; // new Error('Unable to retrieve node status', root);
            var list = [];
            iter.on('entry', function(entry) {
            ...
            iter.on('error', function(e) { // Under normal circumstances this shouldn't happen.
                _self._socket.end();
                callback('Error during LIST(): ' + util.inspect(e));
            });
            iter.on('end', function() {
                if (list.length === 0) {
